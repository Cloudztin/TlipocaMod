...
public class PetalEffect extends AbstractGameEffect {
    ...
    public PetalEffect(float x, float y) {
    ...
    private static final float DUR = 0.4F; private final TextureAtlas.AtlasRegion img;
    private boolean playedSound = false; private boolean forcedAngle = false;
    public PetalEffect(float x, float y, float fAngle) {
        this.img = ImageMaster.DAGGER_STREAK;
        this.x = x - MathUtils.random(320.0F, 360.0F) - this.img.packedWidth / 2.0F;
        this.destY = y;
        this.y = this.destY + MathUtils.random(-25.0F, 25.0F) * Settings.scale - this.img.packedHeight / 2.0F;
